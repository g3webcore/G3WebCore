 ...
 *
 * Most callbacks do no go through here, but get forwarded to G3ContentsClient directly. The
 * messages processed here may originate from the IO or UI thread.
 */
@VisibleForTesting
public class G3ContentsClientCallbackHelper {
    /**
    ...
    private static class OnReceivedErrorInfo {
        final G3ContentsClient.G3WebResourceRequest mRequest;
        final G3ContentsClient.G3WebResourceError mError;

        OnReceivedErrorInfo(G3ContentsClient.G3WebResourceRequest request,
                G3ContentsClient.G3WebResourceError error) {
            mRequest = request;
            mError = error;
        }
    }
    ...
    private static class OnReceivedHttpErrorInfo {
        final G3ContentsClient.G3WebResourceRequest mRequest;
        final G3WebResourceResponse mResponse;

        OnReceivedHttpErrorInfo(
                G3ContentsClient.G3WebResourceRequest request, G3WebResourceResponse response) {
            mRequest = request;
            mResponse = response;
        }
    }
...
//                            info.mRequest, info.mThreatType, info.mCallback);
                ...
                default:
                    throw new IllegalStateException(
                            "G3ContentsClientCallbackHelper: unhandled message " + msg.what);
            }
        }
    }

    public G3ContentsClientCallbackHelper(Looper looper, G3ContentsClient contentsClient) {
    ...
    public void postOnReceivedError(G3ContentsClient.G3WebResourceRequest request,
            G3ContentsClient.G3WebResourceError error) {
    ...
    public void postOnReceivedHttpError(G3ContentsClient.G3WebResourceRequest request,
