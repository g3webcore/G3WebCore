...
public class AndroidProtocolHandler {
    ...
    public static String getMimeType(InputStream stream, String url) {
        ...
        try {
            ...
            if (uri.getScheme().equals(CONTENT_SCHEME)) {
                ...
                if (mimeType == null) {
                    G3HistogramRecorder.recordMimeType(
                            G3HistogramRecorder.MimeType.NULL_FROM_CONTENT_PROVIDER);
                } else {
                    G3HistogramRecorder.recordMimeType(
                            G3HistogramRecorder.MimeType.NONNULL_FROM_CONTENT_PROVIDER);
                }
                return mimeType;
                ...
                if (mimeType == null) {
                    G3HistogramRecorder.recordMimeType(
                            G3HistogramRecorder.MimeType.CANNOT_GUESS_FROM_ANDROID_ASSET_PATH);
                    // Do not return yet, try guessing from the stream.
                } else {
                    G3HistogramRecorder.recordMimeType(
                            G3HistogramRecorder.MimeType.GUESSED_FROM_ANDROID_ASSET_PATH);
                    return mimeType;
                }
            }
        } catch (Exception ex) {
            ...
            G3HistogramRecorder.recordMimeType(
                    G3HistogramRecorder.MimeType.CANNOT_GUESS_DUE_TO_GENERIC_EXCEPTION);
            return null;
        }
        ...
        try {
            ...
            if (mimeType == null) {
                G3HistogramRecorder.recordMimeType(
                        G3HistogramRecorder.MimeType.CANNOT_GUESS_FROM_ANDROID_ASSET_INPUT_STREAM);
            } else {
                G3HistogramRecorder.recordMimeType(
                        G3HistogramRecorder.MimeType.GUESSED_FROM_ANDROID_ASSET_INPUT_STREAM);
            }
            return mimeType;
        } catch (IOException e) {
            G3HistogramRecorder.recordMimeType(
                    G3HistogramRecorder.MimeType.CANNOT_GUESS_DUE_TO_IO_EXCEPTION);
            return null;
        }
    }
