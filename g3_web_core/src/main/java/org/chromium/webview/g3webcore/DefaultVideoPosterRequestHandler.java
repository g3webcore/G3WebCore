...
public class DefaultVideoPosterRequestHandler {
    private static InputStream getInputStream(final G3ContentsClient contentClient)
            throws IOException {
    ...
    private G3ContentsClient mContentClient;

    public DefaultVideoPosterRequestHandler(G3ContentsClient contentClient) {
     ...
     * @return G3WebResourceResponse which caller can get the image if the url is
     * the default video poster URL, otherwise null is returned.
     */
    public G3WebResourceResponse shouldInterceptRequest(final String url) {
        if (!mDefaultVideoPosterURL.equals(url)) return null;

        try {
            return new G3WebResourceResponse("image/png", null, getInputStream(mContentClient));
        } catch (IOException e) {
            Log.e(TAG, null, e);
            return null;
        }
